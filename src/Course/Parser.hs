...
-- data Parser Char = P { parse :: Input -> ParseResult Char }
character =
  P (\i -> case i of
             (h:.t) -> Result t h
             Nil -> ErrorResult Failed
    )
--  P (\(h:.t) -> Result t h)  -- doesn't cover Nil
...
mapParser ::
  ...
  -> Parser b
mapParser f c =
  P (\input1 -> case parse c input1 of
                 ErrorResult e -> ErrorResult e
                 Result input2 v -> Result input2 (f v)
    )
...
--   | Result Input a
bindParser f pa =
  P (\input1 -> case parse pa input1 of
               ErrorResult e -> ErrorResult e
               Result i y -> parse (f y) i
    )
-- bindParser f (P g) = 
--  P (\input1 -> case (g input1) of
--                ErrorResult e -> ErrorResult e
--                Result i y -> parse (f y) i
--    )
...
(>>>) ::
  ...
  -> Parser b
(>>>) pa pb =
--  flbindParser pa (\_ -> pb)
--  bindParser (\_ -> pb) pa
  bindParser (const pb) pa
--(>>>) pa = flbindParser pa . const
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
-- pb ||| pc = P (\i -> case parse pb i of
--                        ErrorResult _ -> case parse pc i of
--                                           ErrorResult e -> ErrorResult e
--                                           Result j v -> Result j v
--                        Result k w -> Result k w
--                        )
pb ||| pc = P (\i -> case parse pb i of
                       ErrorResult _ -> parse pc i
                       r@(Result _ _) -> r
                       )
...
list1 ::
  ...
  -- 0 to many p (call it b) and then
  -- always succed with cons them (a :. b)
list1 p =
  flbindParser p (\a ->
...
satisfy ::
  ...
  -> Parser Char
--satisfy p =
--  character -- satisifies type but not the tests
--  flbindParser character (\c ->
--    if p c then
--        valueParser c
--    else
--        unexpectedCharParser c)
--  flbindParser character (\c -> bool (unexpectedCharParser c) (valueParser c) (p c))
-- bool :: x ->            x->           Bool -> x
-- ???? :: (Char -> x) -> (Char -> x) -> Bool -> x
-- lift3 :: (a -> b -> c -> d) -> (Char -> a) -> (Char -> b) -> (Char -> c) -> (Char -> d)
--  flbindParser character (\c -> lift3 bool unexpectedCharParser valueParser p c)
--  flbindParser character (lift3 bool unexpectedCharParser valueParser p)
--satisfy = (\p -> flbindParser character (lift3 bool unexpectedCharParser valueParser p)) -- but f(g(x) == (f . g) x
--satisfy = (\p -> ((flbindParser character) . (lift3 bool unexpectedCharParser valueParser)) p) -- but (\i -> f i) == f
satisfy = (flbindParser character) . (lift3 bool unexpectedCharParser valueParser)
bool :: a -> a -> Bool -> a
bool x y z = if z then y else x
...
-- sequenceParser (h:.t) =
--   flbindParser h (\a ->
--   flbindParser (sequenceParser t) (\b ->
--   valueParser (a:.b)))
-- sequenceParser (h:.t) =
--   (>>=) h (\a ->
--   (>>=) (sequenceParser t) (\b ->
...
firstNameParser =
  do 
    ...
    l <- list lower
    pure (u:.l)
-- because neither u or l are used on the RHS of arrow, we can also reimplement as:
-- (:.) <$> upper <*> list lower
...
personParser ::
  Parser Person
personParser = 
  Person <$> ageParser <*> space >>> firstNameParser <*> space >>> surnameParser <*> space >>> smokerParser <*> space >>> phoneParser
