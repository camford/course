...
--headOr a list = foldRight (\x xs -> h) a list
...
seqOptional (x:.xs) =
    case x of
        Empty -> Empty
        Full b -> case seqOptional xs of
                    Empty -> Empty
                    Full c -> Full (b :. c)
--seqOptional xs = foldRight unwrap (Full Nil) xs
--                   where unwrap :: Optional a -> Optional (List a) -> Optional (List a)
--                         unwrap Empty _= Empty
--                         unwrap _ Empty = Empty
--                         unwrap (Full y) (Full ys) = Full (y :. ys)
...
--reverse = foldLeft (\y x -> x :. y) Nil

-- THIS EXERCISE IS SUPPOSED TO BE DELETED
